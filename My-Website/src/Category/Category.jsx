...
const Category = ({ addToCart, removeFromCart, cartItems }) => {
  const { id } = useParams();
    ...
    fetch("http://localhost:5000/api/products")
      ...
      .then(data => {
        const filtered = data.filter(
          (p) =>
            p.enabled &&
            normalize(p.category?.category || p.category) === normalize(headData.category)
        );
  ...
  const displayedProducts =
    activeSub === 'all'
      ? products
      : products.filter(p => normalize(p.subcategory) === normalize(activeSub));
  ...
  return (
    <>
      ...
      {/* Subcategory Tabs */}
      <div
        className="catogoryhead2 pt-4"
        style={{ display: 'flex', flexWrap: 'wrap', gap: '1.5rem', backgroundColor }}
      >
        {/* All Tab */}
        <div
          className={`imgcategorydiv ${activeSub === 'all' ? 'active-category' : ''}`}
          onClick={() => setActiveSub('all')}
          style={{ cursor: 'pointer', borderBottom: activeSub === 'all' ? '2px solid red' : '' }}
        >
          <div className="subimg">
            <img
              className="imgcategory mb-3"
              src={headData.image || '/default-category.png'} // category image for "All"
              alt="All"
        ...
        {headData.subcategories.map((sub) => (
          <div
            ...
            className={`imgcategorydiv ${activeSub === sub.name ? 'active-category' : ''}`}
            onClick={() => setActiveSub(sub.name)}
            style={{ cursor: 'pointer', borderBottom: activeSub === sub.name ? '2px solid red' : '' }}
          >
            <div className="subimg">
              <img
                className="imgcategory mb-3"
        ...
        <div className="row breakfasthead d-flex">
          {displayedProducts.length > 0 ? (
            displayedProducts.map((product, index) => {
              const quantity = cartItems?.find(item => item.id === product._id)?.quantity || 0;
              return (
                  ...
                  <div className="card shadow rounded-4">
                    ...
                    <div className="card-body">
                      ...
                      <p className="fw-bold">
                        ...
                        {product.discount && (
                          <span className="breakfast-discount">{product.discount}</span>
                        )}
                      </p>
                      ...
                      {quantity === 0 ? (
                        <button
                          ...
                          onClick={() => addToCart({ ...product, id: product._id, image: product.images[0] })}
                        >
                        ...
                        <div className="quantity-control">
                          ...
                          <button className="qty-btn" onClick={() => addToCart({ ...product, id: product._id, image: product.images[0] })}>+</button>
                        </div>
                      )}
                    </div>
                  </div>
